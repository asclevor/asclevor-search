...
- `api.asclevor.com` — public search API (`{query, limit}` → results with
  `similarity`, `pmid`, `title`, `abstract`, `source`, timings).
- `asclevor.com/docs` — documentation.

## Core function (truth, not aspiration)

- Query is free-text case description; limit 3/5/10; results carry title,
  abstract, similarity 0–1, source URL, PMID, patient id, upstream meta fields.
- URL params `?q=&limit=` restore a search; `/` focuses the search field.
- Latency timings: `embed_ms`, `search_ms`, total.
- Demographics, publication year, and specialty are _extracted_ from record
  text when present — never invented.
